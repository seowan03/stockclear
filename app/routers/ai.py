...
def _rule_based_strategy(product_data: dict[str, Any]) -> dict[str, Any]:
    ...
    if storage_days >= 90 or days_to_sell >= 90 or (stock_qty > 0 and sales_speed <= 0):
        return {"status": "악성재고 위험", "recommended_discount": 30, "comment": "AI 진단을 사용할 수 없어 규칙 기반으로 판단했습니다. 보관 기간과 예상 소진 기간이 길어 할인 또는 처분 계획 검토가 필요합니다."}
    if storage_days >= 45 or days_to_sell >= 45:
        return {"status": "주의", "recommended_discount": 10, "comment": "AI 진단을 사용할 수 없어 규칙 기반으로 판단했습니다. 재고 회전 추이를 모니터링하고 소진 속도 개선을 검토하세요."}
    return {"status": "양호", "recommended_discount": 0, "comment": "AI 진단을 사용할 수 없어 규칙 기반으로 판단했습니다. 현재 지표상 즉시 할인은 필요하지 않습니다."}
...
async def diagnose_inventory(item: InventoryItem, request: Request, current_user: User = Depends(get_current_user)):
    ...
    if ai_result.get("status") == "분석 오류":
        ai_result = _rule_based_strategy(product_dict)
    return {"status": "success", "product_name": item.product_name, "stock_status": ai_result.get("status"), "judgment": ai_result.get("comment")}
